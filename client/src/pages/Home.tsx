...
import { ArrowRight, ArrowUpRight, Blocks, Check, ChevronDown, ClipboardCopy, Code2, Headphones, Image as ImageIcon, Layers3, LockKeyhole, Megaphone, MessageCircle, Palette, Play, Settings2, ShieldCheck, Sparkles, SlidersHorizontal, Terminal, WandSparkles, X, Zap } from "lucide-react";
...
const plans = [
  { name: "Başlangıç (Free)", price: "₺0", note: "başlangıç", label: "FREE PLAN · KAMPANYA!", detail: "Temel komutlar ve hazır modüllerle sade bir bot başlangıcı.", performance: "Temel", score: 1, tone: "plain", icon: Terminal, notice: "Bu sunucuda Göktürk Labs tarafından hazırlanan Discord botu kullanılmaktadır. Hizmet hakkında bilgi almak için göktürklabs.dev.tc adresini inceleyebilirsiniz.", features: ["Temel komutlar", "Temel otomasyon özellikleri", "Hazır modüllerle kurulum", "3 + 1 modül hakkı · KAMPANYA!", "Modüllü seçenek: Göktürk Labs sunucusunda çalışma", "Kod paylaşımı yok", "İsim/logo/kapak düzenleme yok", "Zorunlu: Sunucu adı/açıklamasında \"— By Göktürk Labs\" + link, ayrıca banner'da da \"Powered by Göktürk Labs\" + link (ikisi de kaldırılamaz)"] },
  { name: "Dengeli", price: "₺50", note: "tek sefer", label: "DENGELİ · 4 MODÜL", detail: "Başlangıç kapsamını moderasyon, loglama ve 4 modüllük çalışma alanıyla büyütür.", performance: "Dengeli", score: 2, tone: "plain", icon: Code2, features: ["Başlangıç paketindeki tüm özellikler", "Moderasyon ve loglama", "Gelişmiş otomasyon sistemleri", "4 modül hakkı", "Modüllü seçenek: Göktürk Labs sunucusunda çalışma", "Kod paylaşımı yok", "Custom bot seçeneği yok", "İsim, logo, kapak görseli düzenleme var", "Açıklama + banner'da zorunlu: \"Powered by Göktürk Labs\" + link (kaldırılamaz)"] },
  { name: "Gelişmiş", price: "₺150", note: "tek sefer", label: "GELİŞMİŞ · 8 MODÜL", detail: "Dengeli paketinin üzerine özel komutlar, daha geniş modül sınırı ve custom seçeneği eklenir.", performance: "Yüksek", score: 3, tone: "featured", icon: Sparkles, features: ["Dengeli paketindeki tüm özellikler", "Ticket ve özel komutlar", "Gelişmiş sunucu sistemleri", "8 modül hakkı", "Modüllü seçenek: Göktürk Labs sunucusunda çalışma", "Custom bot seçeneği", "Custom seçenekte kod paylaşımı var", "Custom seçenekte kullanıcı veya Göktürk Labs sunucusu", "İsim, logo, kapak görseli düzenleme", "\"Powered by\" ibaresi sorunsuz kaldırılabilir"] },
  { name: "Pro", price: "₺350", note: "tek sefer", label: "PRO / ÖNCELİKLİ · 12 MODÜL", detail: "Gelişmiş kapsamına en geniş modül sınırı, web paneli ve öncelikli destek eklenir.", performance: "Öncelikli", score: 4, tone: "warm", icon: WandSparkles, features: ["Gelişmiş paketindeki tüm özellikler", "12 modül hakkı", "Modüllü seçenek: Göktürk Labs sunucusunda çalışma", "Custom bot seçeneği", "Custom seçenekte kod paylaşımı var", "Custom seçenekte kullanıcı veya Göktürk Labs sunucusu", "İleri seviye özel sistemler", "Web panel desteği ve öncelikli teknik destek", "İsim, logo, kapak görseli düzenleme", "\"Powered by\" ibaresi sorunsuz kaldırılabilir"] },
];
...
const extraSystems = [
  ...
  { title: "Çekiliş sistemi", detail: "Katılım, süre ve kazanan akışını kolaylaştıran modül.", icon: Sparkles, cover: "cover-amber" },
  { title: "Destek & talep", detail: "Kullanıcı taleplerini düzenli kanallara ve akışlara ayır.", icon: Headphones, cover: "cover-orange" },
...
function CommunityButton({ label = "Topluluk & destek sunucusu" }: { label?: string }) {
  return <a href={COMMUNITY_URL} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm font-semibold text-[#ffd28b] hover:text-white"><DiscordMark className="h-4 w-4" />{label}<ArrowUpRight className="h-4 w-4" /></a>;
}

function FeatureIcon({ feature }: { feature: string }) {
  if (feature === "İsim/logo/kapak düzenleme yok" || feature === "Kod paylaşımı yok" || feature === "Custom bot seçeneği yok") return <X className="mt-0.5 h-4 w-4 shrink-0 text-[#ffb547]" aria-hidden="true" />;
  if (feature.startsWith("Zorunlu:") || feature.startsWith("Açıklama + banner'da zorunlu:")) return <LockKeyhole className="mt-0.5 h-4 w-4 shrink-0 text-[#ffb547]" aria-hidden="true" />;
  return <Check className="mt-0.5 h-4 w-4 shrink-0 text-[#ffb547]" aria-hidden="true" />;
}
...
function PlanOrder({ plan }: { plan: (typeof plans)[number] }) {
  ...
  const orderMessage = `Merhaba, ${plan.name} (${plan.price}) paketi için sipariş vermek istiyorum.`;
...
function PlanCard({ plan }: { plan: (typeof plans)[number] }) {
  ...
  return <article className={`new-plan-card ${plan.tone === "featured" ? "new-plan-card-featured" : ""} ${plan.tone === "warm" ? "new-plan-card-warm" : ""}`}><div className="flex items-center justify-between"><span className="new-plan-label">{plan.label}</span><Icon className="h-5 w-5 text-[#ffb547]" /></div><h3>{plan.name}</h3><p className="new-plan-detail">{plan.detail}</p><div className="new-plan-price"><strong>{plan.price}</strong><span>/ {plan.note}</span></div><div className="plan-performance"><div><span>Performans</span><strong>{plan.performance}</strong></div><div className="performance-meter">{[1, 2, 3, 4].map(level => <span key={level} className={level <= plan.score ? "is-on" : ""} />)}</div></div><div className="new-plan-line" /><div className="space-y-3">{plan.features.map(feature => <div className="flex items-start gap-2.5 text-sm text-[#b9b8c4]" key={feature}><FeatureIcon feature={feature} /><span>{feature}</span></div>)}</div>{plan.notice && <div className="plan-mandatory-notice"><Megaphone className="h-4 w-4 shrink-0 text-[#ffb547]" /><div><strong>Zorunlu sunucu duyurusu</strong><p>{plan.notice}</p></div></div>}<PlanOrder plan={plan} /></article>;
}
...
export default function Home() {
  ...
  return <main className="new-site min-h-screen overflow-x-hidden bg-[#08080b] text-[#f7f7fb]"><ClickBurst /><header className="new-nav"><div className="new-nav-inner"><a href="#top" className="flex items-center gap-3" aria-label="Göktürk Labs ana sayfa"><img src={LOGO_URL} alt="Göktürk Labs logosu" className="h-8 w-8 object-contain" /><span className="font-display text-[15px] font-semibold tracking-[-0.02em]">Göktürk <span className="text-[#ffb547]">Labs</span></span></a><nav className="hidden items-center gap-7 text-[13px] text-[#aaa9b5] md:flex"><a href="#nasil">Nasıl çalışır?</a><a href="#moduller">Modüller</a><a href="#paketler">Paketler</a><a href="#sss">SSS</a><a href={COMMUNITY_URL} target="_blank" rel="noreferrer" className="community-nav-cta"><DiscordMark className="h-4 w-4" /><span>Topluluk &amp; destek</span><ArrowUpRight className="h-4 w-4" /></a></nav></div></header>
    <section id="top" className="new-hero"><div className="hero-glow hero-glow-violet" /><div className="hero-glow hero-glow-blue" /><div className="hero-grid" /><div className="motion-hero-copy relative z-10 mx-auto max-w-6xl px-5 pb-24 pt-32 sm:pt-40"><div className="new-badge"><span className="new-badge-dot" /> GÖKTÜRK LABS / DISCORD BOT <span className="new-badge-divider" /> <span className="new-badge-link">Fikrini anlat <ArrowRight className="h-3.5 w-3.5" /></span></div><h1 className="mt-8 font-display text-5xl font-medium leading-[1.02] tracking-[-0.075em] text-white sm:text-7xl lg:text-[88px]">Botun sürekli<br /><span className="hero-gradient-text">çalışsın.</span></h1><p className="mx-auto mt-7 max-w-2xl text-base leading-7 text-[#a5a4b0] sm:text-lg">Botunu güvenilir bir çalışma düzenine al. İhtiyacını Discord’da anlat; paketi seçelim, botun seçtiğin sunucuda gece gündüz çalışsın ve desteği yanında olsun.</p><div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row"><a href="#paketler" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#ffb547] px-5 text-sm font-semibold text-[#17120a] shadow-[0_12px_34px_rgba(255,181,71,0.2)] transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#ffd18a]"><span>Paketini seç</span><ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></a><a href="#nasil" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-white/12 px-5 text-sm font-semibold text-white hover:bg-white/[0.05]"><Play className="h-4 w-4 fill-current" /> Nasıl çalışır?</a></div><div className="mt-6 flex justify-center"><CommunityButton label="Topluluk & destek sunucusuna kat" /></div><div className="mt-14 flex flex-wrap items-center justify-center gap-x-7 gap-y-3 text-[11px] font-medium uppercase tracking-[0.12em] text-[#777682]"><span className="inline-flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-[#ffb547]" /> Fikrin sende</span><span className="inline-flex items-center gap-2"><Code2 className="h-4 w-4 text-[#ffb547]" /> Üretim bizde</span><span className="inline-flex items-center gap-2"><Headphones className="h-4 w-4 text-[#ffb547]" /> Destek yanında</span></div><aside className="hero-console" aria-label="Bot çalışma durumu"><div className="hero-console-top"><span><i /> ÇALIŞMA İSTASYONU</span><small>LIVE / 24—7</small></div><div className="hero-console-main"><div className="hero-console-orbit"><div className="hero-console-core"><span>GL</span></div></div><div className="hero-console-copy"><strong>Göktürk Labs</strong><span>Bot çalışma hattı</span></div></div><div className="hero-console-stats"><div><small>Durum</small><strong><i /> Çevrimiçi</strong></div><div><small>İşlem</small><strong>Hazır modüller</strong></div><div><small>Destek</small><strong>Discord DM</strong></div></div><div className="hero-console-line"><span /><span /></div></aside></div></section>
